...
New profiles immediately receive all currently active character names without restarting. Thumbnail Settings also provides a per-profile lock that prevents accidental right-drag movement and mouse resizing. Stable version 2.0.0 also hardens forward and backward group cycling: unavailable clients are skipped with a bounded search, minimized targets are activated reliably, and stale minimize actions cannot interfere with a rapid switch. Version 2.0.1 aligns the complete Thumbnail Settings page on a consistent two-column grid. Version 2.0.2 automatically stores thumbnail and EVE-window layouts per profile and applies profile settings immediately without restarting. Version 2.1.0 adds the selectable Classic, Modern Dark and Modern Light interfaces and removes the configurable thumbnail minimum size. Version 2.1.1 rebuilds both modern interfaces as independent card-based layouts and adds automated screenshots of the compiled Windows GUI for visual release review. Version 2.2.0 completes the approved modern design in dark and light, adds a live thumbnail preview and a separate detail view while keeping every existing setting accessible; Classic remains available.
...
| Option | Description | 
| --- | --- |
| Design | Switches immediately between **Classic**, **Modern Dark** and **Modern Light**. The selected interface is retained for the next start. |
