...
| Forward hotkey <br> Backward hotkey | Activates the next or previous available character in the list. Use **Capture…** to record the actual keyboard key; for example, Numpad 3 is saved as `Numpad3` instead of the normal number-row `3`. Manual entry remains available for mouse hotkeys and advanced syntax. |

### **Profile Settings - Character Hotkeys**

Assign a separate hotkey to activate a specific character directly. No cycle group is required. Character names and hotkeys must occupy matching rows, and the same hotkey may only be assigned once.
