...
> **EVE-X-Preview Reloaded** continues this MIT-licensed project with a German/English interface, GitHub update notifications, and portable ZIP releases.

**Project website:** [Features, quick start, current releases, roadmap and support](https://savox76.github.io/EVE-X-Preview-reloaded/)

### Reloaded portable version

1. Download `EVE-X-Preview-Reloaded-*-portable.zip` from the [Releases page](https://github.com/Savox76/EVE-X-Preview-reloaded/releases).
2. Extract the complete ZIP into a writable folder, for example inside Documents.
3. Start `EVE-X-Preview-Reloaded.exe` from that folder.
4. Keep the `locales` folder beside the program. Your personal `EVE-X-Preview.json` is created in the same folder and is retained during updates.

The language can be changed under **Global Settings / Allgemeine Einstellungen**. The tray menu can check for updates manually; the application also notifies you automatically when a newer GitHub release is available. No installer or separate EXE download is published.

Color fields provide a native Windows color palette and still accept hexadecimal or RGB input. Character names from running EVE clients are saved automatically in the active profile for hotkeys and individual colors. The selectable next development packages are documented in the [`development master plan`](docs/MASTERPLAN.md); evaluated ideas and bug reports from the upstream project remain tracked in the [`original-repository backlog`](docs/ORIGINAL_REPO_BACKLOG.md).

New profiles immediately receive all currently active character names without restarting. Thumbnail Settings also provides a per-profile lock that prevents accidental right-drag movement and mouse resizing. Stable version 2.0.0 also hardens forward and backward group cycling: unavailable clients are skipped with a bounded search, minimized targets are activated reliably, and stale minimize actions cannot interfere with a rapid switch. Version 2.0.1 aligns the complete Thumbnail Settings page on a consistent two-column grid. Version 2.0.2 automatically stores thumbnail and EVE-window layouts per profile and applies profile settings immediately without restarting. Version 2.0.3 removes the configurable minimum thumbnail size so previews can be scaled freely down to the smallest positive size Windows supports. Version 2.0.4 replaces the parallel custom-color text lists with one scrollable row per character; clicking a color field opens the Windows palette directly. Version 2.0.5 clarifies the difference between direct character hotkeys and optional cycle groups directly in the settings UI. Version 2.0.6 adds an automatically maintained Default cycle group containing every detected character in the profile. Version 2.0.7 adds unambiguous keyboard capture for cycle-group hotkeys, including distinct Numpad keys. Version 2.0.8 makes hotkey replacement and clearing atomic and extends capture to function keys and complete modifier combinations. Version 2.0.9 corrects the swapped forward and backward cycle directions.
...
1. Download the portable ZIP from https://github.com/Savox76/EVE-X-Preview-reloaded/releases
2. Extract the complete ZIP to Desktop, Documents, or another writable folder outside the Windows Program Files folders.
3. Start EVE-X-Preview-Reloaded.exe. It creates an `EVE-X-Preview.json` file in the same folder. This file contains all your settings.
...
`Important Note:`  <br> `Profile settings are applied immediately. Changing a global setting that affects the complete application can still trigger one automatic reload when the settings window is closed.`
...
| Save Client Positions | Creates an immediate snapshot of all EVE client positions. Positions are also captured automatically in the active profile; **Restore Client Positions** controls whether they are restored. |
| Save Thumbnail Positions | Creates an immediate snapshot of the thumbnail layout. Moving or resizing thumbnails is also saved automatically in the active profile. |
...
| Option | Description | 
| --- | --- |
| Suspend Hotkeys - Hotkey | Allows you to suspend all active hotkeys. Use **Capture hotkey…** for function keys or complete combinations and **Clear** to remove the assignment safely. The tray icon changes while hotkeys are suspended. |
...
Each detected character has one row with separate fields for the active border, text and inactive border colors. Click a color field directly to open the native Windows color palette. New and Delete allow manual maintenance; detected EVE clients are still added automatically.
...
### **Profile Settings - Cycle Groups**

Every profile receives an automatically maintained **Default** group containing all detected characters. Assign only its forward and/or backward hotkey to start cycling immediately. Characters that are not currently running are skipped. Additional groups remain optional for a different selection or order, and no cycle group sends gameplay input to EVE.

| Option | Description |
| --- | --- |
| Select Group | Default is maintained automatically and cannot be deleted. Additional independent cycle lists can be created and deleted. |
| Characters in cycle order | Enter one character name per line. The activation sequence runs from top to bottom. |
| Forward hotkey <br> Backward hotkey | Activates the next or previous available character in the list. Use **Capture hotkey…** to record the complete key or combination, including function and Numpad keys. **Clear** removes the assignment without an error. Manual entry remains available for mouse hotkeys and advanced syntax and is applied after leaving the field. |

### **Profile Settings - Character Hotkeys**

Assign a separate hotkey to activate a specific character directly. No cycle group is required. Character names and hotkeys must occupy matching rows, and the same hotkey may only be assigned once. Click the desired hotkey row and use **Capture hotkey…** or **Clear** so the whole assignment is changed atomically.
...
EVE-X-Preview was created by [g0nzo83](https://github.com/g0nzo83), also known as "*John Xer*" in EVE Online. EVE-X-Preview Reloaded continues that MIT-licensed work in [Savox76/EVE-X-Preview-reloaded](https://github.com/Savox76/EVE-X-Preview-reloaded).
